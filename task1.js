...
// begin: ../../common.blocks/grid/__cell/_format/grid__cell_format_airline.bh.js
	...
	bh.match('grid__cell_format_airline', function(ctx) {
		...
		ctx.attr('data-bem', JSON.stringify({ grid__cell_format_airline: { name: data.name } })).content({
			block: 'icon',
			cls: 'grid__cell_format_airline-logo',
			attrs: { 'data-name': data.name },
			url: 'http://0.omg.io/wego/image/upload/c_fit,w_70,h_30/flights/airlines_rectangular/' + data.fs + '.png'
		});
	});
			...
			get: function(params) {
				...
				if (false && window.airportStatusData != null) { //Try to request ajax. Resolve with saved data on error.
					...
					$.ajax({
						dataType: "jsonp",
						url: [
							url, this._params.airport, this._params.mode,
							moment.getFullYear(), moment.getMonth() + 1, moment.getDate(), moment.getHours()
